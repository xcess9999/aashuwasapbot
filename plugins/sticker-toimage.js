...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    const notStickerMessage = `ᴛᴀɢ ᴀ sᴛɪᴄᴋᴇʀ ᴡɪᴛʜ ᴄᴏᴍᴍᴀɴᴅ *_"${usedPrefix + command}"_*`
    ...
    if (/sticker/.test(q.mediaType)) {
        ...
        im.stdin.end()
        im.on('exit', () => {
            conn.sendFile(m.chat, Buffer.concat(bufs), 'image.png', null, m)
        })
    } else throw notStickerMessage
}
handler.help = ['toimage (tag sticker)']
